...
## 1. 목표 제품 범위 (현재 구현 현황 반영)

- [v] **전문적인 디자인 시스템 적용**: 'Quiet Paper' 테마 기반의 모던 모바일 UI (390x800 프레임)
- [v] **다중 캐릭터 라인업**: 반달곰, 기린, 코알라, 토끼, 드래곤 등 15종 캐릭터 지원
- [v] **캐릭터 성장 시스템**: 시간 경과에 따른 4단계 진화 (알 -> 아기 -> 어린이 -> 성체)
- [v] **캐릭터 커스터마이징**: 이름 변경, HUE(색조) 조절, 액세서리(모자, 안경 등), 룸 패턴 설정
- [v] **일기 기록 및 AI 답장**: 키워드 분석 기반의 감성적인 AI 답장 생성
- [v] **채팅형 추억 보관소**: SNS 대화창 형식의 일기/답장 히스토리 관리
- [v] **로컬 데이터 유지**: localStorage를 통한 설정 및 추억 데이터 영구 보관

### MVP 상세 항목 체크리스트
- [ ] 게스트/소셜 로그인 진입 구조 (UI만 구현됨)
- [v] 연령 모드 선택
- [v] 동의 화면 (통합됨)
- [v] 캐릭터 생성 및 다중 캐릭터 선택 (반달곰, 기린, 코알라, 토끼, 드래곤 등)
- [v] 페르소나 선택: 친구, 가족, 선생님, 멘토
- [v] 말투 선택 (통합됨)
- [v] 기본 아바타 파츠 선택 (액세서리 기능)
- [v] 기본 패턴 선택과 색상 변경
- [v] 오늘의 감정 선택
- [v] 한 줄 교환일기 작성
- [v] AI 추가 질문 1개 생성 (Mock AI)
- [v] AI 답장 생성 (Mock AI)
- [v] 일기 저장/조회
- [v] 날짜별 일기 목록 (채팅형)
- [ ] 기억 후보 제안/승인/삭제
- [ ] 기본 안전 필터 (도메인 정의 필요)
- [v] Mock AI Gateway 우선 (완료)
